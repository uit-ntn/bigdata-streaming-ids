...
MODEL_CONFIG = {
    ...
    "note": (
        "Decision Tree is used as a non-linear baseline model. "
        "It is easier to interpret than many ensemble models and can provide feature importance."
    ),
}
...
def build_model():
    print_section("3. BUILD DECISION TREE MODEL")
...
def train_model(model, X_train, y_train):
    ...
    model_path = MODEL_DIR / "decision_tree.joblib"
...
def evaluate_model(y_test, y_pred, y_proba, train_time, prediction_time, model):
...
def save_figure_custom(filename: str, bbox_inches=None):
    plt.tight_layout()
    save_path = FIGURE_DIR / filename
    plt.savefig(save_path, dpi=150, bbox_inches=bbox_inches)
    plt.close()
    print(f"[OK] Saved figure: {save_path}")
...
def plot_threshold_metrics(threshold_df):
    ...
    plt.title("Threshold Analysis - Decision Tree")
...
def plot_feature_importance(model, feature_names):
    if feature_names is None:
        print("[WARNING] Skip feature importance because feature_names is missing.")
        return
    ...
    plt.title("Top 25 Feature Importance - Decision Tree")
    plt.xlabel("Importance")
    plt.ylabel("Feature")

    save_figure("08_feature_importance_barh.png")


def plot_feature_importance_lollipop(model, feature_names):
    if feature_names is None:
        return
    ...
    plt.plot(importance_df["importance"], y_pos, "o")

    plt.yticks(y_pos, importance_df["feature"])
    plt.title("Top 20 Feature Importance - Lollipop Chart")
    plt.xlabel("Importance")
    plt.ylabel("Feature")

    save_figure("09_feature_importance_lollipop.png")


def plot_tree_depth_summary(model):
    tree_summary = {
        "tree_depth": model.get_depth(),
        "num_leaves": model.get_n_leaves(),
    ...
    save_json(tree_summary, RESULT_DIR / "tree_structure_summary.json")

    rows = [
        ["Tree depth", tree_summary["tree_depth"]],
        ["Number of leaves", tree_summary["num_leaves"]],
        ["Number of nodes", tree_summary["num_nodes"]],
        ["Number of classes", tree_summary["num_classes"]],
    ]
    ...
    save_path = FIGURE_DIR / "10_tree_structure_summary_table.png"
...
def plot_decision_tree_preview(model, feature_names):
    if feature_names is None:
        print("[WARNING] Skip tree preview because feature_names is missing.")
        return
    ...
    plt.title("Decision Tree Preview - First 3 Levels")

    save_path = FIGURE_DIR / "11_decision_tree_preview_depth_3.png"
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
    ...
    tree_text_path = RESULT_DIR / "decision_tree_rules_preview.txt"
    tree_text_path.write_text(tree_text, encoding="utf-8")

    print(f"[OK] Saved tree rules preview: {tree_text_path}")
...
def plot_metrics_summary_table(metrics):
    selected_metrics = [
        ...
        ["Number of leaves", metrics["num_leaves"]],
    ...
    plt.title("Decision Tree Metrics Summary")
...
def create_visualizations(model, y_test, y_pred, y_proba, cm, metrics, feature_names):
    ...
    plot_feature_importance_lollipop(model, feature_names)
    plot_tree_depth_summary(model)
    plot_decision_tree_preview(model, feature_names)
...
def generate_report_notes(metrics):
    ...
    lines = [
        ...
        f"Criterion: {MODEL_CONFIG['parameters']['criterion']}",
        f"Max depth: {MODEL_CONFIG['parameters']['max_depth']}",
        f"Min samples split: {MODEL_CONFIG['parameters']['min_samples_split']}",
        f"Min samples leaf: {MODEL_CONFIG['parameters']['min_samples_leaf']}",
        ...
        f"Number of leaves: {metrics['num_leaves']}",
        "",
    ...
    report_path = RESULT_DIR / "decision_tree_report_notes.md"
